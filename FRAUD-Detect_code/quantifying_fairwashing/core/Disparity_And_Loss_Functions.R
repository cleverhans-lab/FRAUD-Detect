...
computeKL_riskScore <- function(augmented_data, disparity_measure, protected_class) {
 # Provides a simple wrapper function to compute the disparity measure associated with 
 # user specified choice. 
 #
 # Input
 ...
 result_BFPC = mean( augmented_data %>% filter(A == protected_class & Y == 1) %>% pull(threshold_classifier) ) - mean( augmented_data %>% filter(A != protected_class & Y == 1) %>% pull(threshold_classifier) )
 result_BFNC = mean( augmented_data %>% filter(A == protected_class & Y == 0) %>% pull(threshold_classifier) ) - mean( augmented_data %>% filter(A != protected_class & Y == 0) %>% pull(threshold_classifier) )
 result = result_BFPC + result_BFNC
 return(result)
}

compute_confusion_matrix <- function(y, y_true) {
    # compute tp, tn, fn, fp
    tp = sum((y==1)&(y_true==1))
    tn = sum((y==0)&(y_true==0))
    fp = sum((y==1)&(y_true==0))
...
getKL <- function(augmented_data, disparity_measure, protected_class) {
 
 augmented_data <- augmented_data %>% mutate(threshold_classifier = ifelse(riskScore >= Z, 1, 0))
 # get prediction and true label

 ## majority group
 y_maj = augmented_data %>% filter(A != protected_class) %>% pull(Y)
 u_maj = augmented_data %>% filter(A != protected_class) %>% pull(threshold_classifier)

 ## minority group
 y_min = augmented_data %>% filter(A == protected_class) %>% pull(Y)
 u_min = augmented_data %>% filter(A == protected_class) %>% pull(threshold_classifier)

 # get confusion matrices
 cm_maj = compute_confusion_matrix(u_maj, y_maj)
 cm_min = compute_confusion_matrix(u_min, y_min)

 # compute kl
 kl_tp <- cm_maj$tp * log(cm_maj$tp/cm_min$tp)
 kl_tn <- cm_maj$tn * log(cm_maj$tn/cm_min$tn)
 kl_fp <- cm_maj$fp * log(cm_maj$fp/cm_min$fp)
 kl_fn <- cm_maj$fn * log(cm_maj$fn/cm_min$fn)

 kl <- kl_tp + kl_tn + kl_fp + kl_fn
