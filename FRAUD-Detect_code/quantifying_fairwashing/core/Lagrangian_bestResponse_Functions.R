...
compute_CLambda_extremes <- function(augmented_data, lagrange_mult, lagrange_mult_kl, protected_class, 
  ...
  if (disparity_measure == "SP") {
    augmented_data <- augmented_data %>%
      mutate(
        Clambda = case_when(
          A == protected_class ~ (1/disparity_moments$p0) + lagrange_mult*C + lagrange_mult_kl*C, 
          A != protected_class ~ -(1/disparity_moments$p1) + lagrange_mult*C + lagrange_mult_kl*C
        ))
  } else if (disparity_measure == "BFPC") { 
    augmented_data <- augmented_data %>%
      mutate(
